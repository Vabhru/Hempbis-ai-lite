...
const App: React.FC = () => {
  ...
  const getTargetPersonaIdFromSuggestion = (suggestionText: string): string | null => {
  ...
  const handleAiSuggestionClick = async (suggestionText: string, suggestionForPersonaId?: string) => {
    const targetPersonaIdFromSuggestion = getTargetPersonaIdFromSuggestion(suggestionText);
    let personaForThisMessage = activePersona; // Default to current state

    if (targetPersonaIdFromSuggestion && targetPersonaIdFromSuggestion !== activePersonaId) {
        // setInputLoading(true); // handlePersonaChange will manage its own loading state
        ...
        if (personaChangedSuccessfully) {
            personaForThisMessage = targetPersonaObject; // Use the new persona for the subsequent message
        } else {
            // Error message added by handlePersonaChange. It also sets inputLoading to false.
            return; // Don't send message if switch failed
        }
    }
    // Pass the determined personaForThisMessage to handleSendMessage
    await handleSendMessage(suggestionText, undefined, personaForThisMessage);
  };
    ...
    return (
      <div className="p-4 md:p-6 space-y-8 animate-fadeIn flex-grow overflow-y-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-slate-100 mb-2">Welcome to Hempbis AI</h2>
